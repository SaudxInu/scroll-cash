...
let web3, tornado, circuit, proving_key, groth16, erc20, SENDER_ACCOUNT, netId
...
async function deposit({ currency, amount }) {
  ...
  if (currency === 'eth') {
    ...
    await printETHBalance({ address: SENDER_ACCOUNT, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await tornado.methods.deposit(toHex(deposit.commitment)).send({ value, from: SENDER_ACCOUNT, gas: 2e6 })
    await printETHBalance({ address: tornado._address, name: 'Tornado' })
    await printETHBalance({ address: SENDER_ACCOUNT, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: tornado._address, name: 'Tornado' })
    await printERC20Balance({ address: SENDER_ACCOUNT, name: 'Sender account' })
    const decimals = isLocalRPC ? 18 : config.deployments[`netId${netId}`][currency].decimals
    const tokenAmount = isLocalRPC ? TOKEN_AMOUNT : fromDecimals({ amount, decimals })
    if (isLocalRPC) {
      console.log('Minting some test tokens to deposit')
      await erc20.methods.mint(SENDER_ACCOUNT, tokenAmount).send({ from: SENDER_ACCOUNT, gas: 2e6 })
    }

    const allowance = await erc20.methods.allowance(SENDER_ACCOUNT, tornado._address).call({ from: SENDER_ACCOUNT })
    console.log('Current allowance is', fromWei(allowance))
    if (toBN(allowance).lt(toBN(tokenAmount))) {
      console.log('Approving tokens for deposit')
      await erc20.methods.approve(tornado._address, tokenAmount).send({ from: SENDER_ACCOUNT, gas: 1e6 })
    }

    console.log('Submitting deposit transaction')
    await tornado.methods.deposit(toHex(deposit.commitment)).send({ from: SENDER_ACCOUNT, gas: 2e6 })
    await printERC20Balance({ address: tornado._address, name: 'Tornado' })
    await printERC20Balance({ address: SENDER_ACCOUNT, name: 'Sender account' })
  }
...
async function generateMerkleProof(deposit) {
  ...
  const events = await tornado.getPastEvents('Deposit', { fromBlock: 1721000, toBlock: 'latest' })
  const leaves = events
...
async function withdraw({ deposit, currency, amount, recipient, relayerURL, refund = '0' }) {
  ...
  if (relayerURL) {
    ...
    SENDER_ACCOUNT = account.address
    //
    await tornado.methods.withdraw(proof, ...args).send({ from: SENDER_ACCOUNT, value: refund.toString(), gas: 1e6 })
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    ETH_AMOUNT = 1e18
    TOKEN_AMOUNT = 1e19
    SENDER_ACCOUNT = (await web3.eth.getAccounts())[0]
  } else {
  ...
  if (inBrowser) {
    ...
    program
      ...
      .action(async () => {
        ...
        await withdraw({ deposit: parsedNote.deposit, currency, amount, recipient: SENDER_ACCOUNT, relayerURL: program.relayer })
        ...
        await withdraw({ deposit: parsedNote.deposit, currency, amount, recipient: SENDER_ACCOUNT, refund: '0.02', relayerURL: program.relayer })
      })
    try {
